...
window.addEventListener('load', function() {
	...
	socket.on('START_GAME', function(msg) {
	   ...
	   if(window.unityInstance!=null)
		  {
			...
			window.unityInstance.SendMessage ('NetworkManager', 'OnSTARTGAME', msg);
	  ...
	  socket.on('NEXT_PLAYER', function(msg) {
	   ...
	   if(window.unityInstance!=null)
		  {
			...
			window.unityInstance.SendMessage ('NetworkManager', 'OnNextPlayer', msg);
	   ...
	   //NEXTPLAYER
	   socket.on('PLAY_CHOICE', function(msg) {
		//var currentUserAtr = socket_id+':'+msg;     		
		//var currentUserAtr = socket_id+':'+msg;
	   ...
	   if(window.unityInstance!=null)
		  {
			...
			window.unityInstance.SendMessage ('NetworkManager', 'OnEmitReadyToPlay', msg);
	...
	socket.on('UPDATE_GAMEOBJECT_MOVE_AND_ROTATE', function(gameObject_id,position,rotation) {
		var currentGameobject = gameObject_id+':'+position+':'+rotation;
			
		if(window.unityInstance!=null)
	   {
		  window.unityInstance.SendMessage ('NetworkManager', 'OnUpdateGameObjectMoveAndRotate',currentGameobject);
	   }
