...
io.on('connection', function(socket){
  ...
  var currentUser;
	...
	socket.on('NEXTPLAYER', function (_pack)
	{
	  //console.log('_pack# '+_pack);
	  var pack = JSON.parse(_pack);	
		 ...
		 {
			//emit back to NetworkManager in Unity by client.js script
			socket.broadcast.emit('NEXT_PLAYER',pack.msg);
		}

		
	});
	socket.on('EMITPLAYCHOICE', function (_pack)
	{
	  //console.log('_pack# '+_pack);
	  var pack = JSON.parse(_pack);	
		 ...
		 {
			//emit back to NetworkManager in Unity by client.js script
			socket.broadcast.emit('PLAY_CHOICE',pack.msg);
		}

		
	});

		//create a callback fuction to listening EmitStartGame() method in NetworkMannager.cs unity script
		socket.on('STARTGAME', function (_pack)
		{
		  //console.log('_pack# '+_pack);
		  var pack = JSON.parse(_pack);	
	
			console.log('Try to launch Game# '+socket.id+": "+pack.msg);
			// if(currentUser)
	  		// {
	
	  
				socket.broadcast.emit('START_GAME', pack.msg);
	...
	socket.on('LOGIN', function (_data)
	{
        ...
        currentUser = {
				   ...
				   rotation:'0',
			       id: clients.length +1,//socket.id,//alternatively we could use socket.id
		...
		console.log('[INFO] currentUser.position '+currentUser.position);	
		 ...
		 console.log('ID Player' + currentUser.id);
	...
	socket.on('GAMEOBJECT_MOVE_AND_ROTATE', function (_data)
	{
	  var data = JSON.parse(_data);	
	  ...
	  if(currentUser)
	  {
	   	...
	   	// send current user position and  rotation in broadcast to all clients in game
       	socket.broadcast.emit('UPDATE_GAMEOBJECT_MOVE_AND_ROTATE', data.gameObject_id,data.position,data.rotation);
